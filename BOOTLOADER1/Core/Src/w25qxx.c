...
////////////////////////////////////////////////////////////////////////////////// 	
//使用共用体存储浮点数到外部flash
//https://blog.csdn.net/qq_44852376/article/details/128387527
...
{ 
	...
	secremain=4096-secoff;//扇区剩余空间大小   
 	printf("ad:%X,nb:%X\r\n",WriteAddr,NumByteToWrite);//测试用:ad地址，nb写入字节数
...
{
    int count = 0;
    while (*arr != '\0')  // 以'\0'为结尾的C字符串，通过判断是否为'\0'来确定数据个数
    {   
		count++;
        arr++;
    }
...
//P1 浮点数地址
//addr 要写入的flash地址
int W25Q128_Write_float(float f,int addr)
{ 	
	uint8_t DataBuffer[4];
	DataTrans.f=f;//接收字符串转浮点数给共用体的浮点成员
		for (int i = 0; i < 4; i++)//用零时缓存复制共用体内容
		{
			DataBuffer[i] = DataTrans.ch[i];
		}
		W25QXX_Write((uint8_t*)DataBuffer,addr,4);//写入flash
		printf("flash写入浮点：%f\r\n",f);//打印调试
	return 0;
}
...
//从flash取出浮点数
//addr 浮点数在flash中的地址
float W25Q128_Read_float(int addr)
{	float rvalue;
	uint8_t datatemp[5];//浮点数临时缓存
	W25QXX_Read(datatemp,addr,4);//读出刚刚写入的内容到零时缓存datatemp
		//把临时缓存的内容以char存入共用体
		for(int i=0; i<4; i++)
		{
		DataTrans.ch[i] = datatemp[i];
		}
		rvalue = DataTrans.f;	//以float的格式取出共用体内容		
	return rvalue;
}
