...
#endif
volatile uint8_t BC260_rec_flag = 0; //BC260�?帧数据接收完成标�?
//串口1（蓝牙）（调试口�?
volatile uint8_t rx1_len = 0;  //USART1接收�?帧数据的长度
volatile uint8_t rec1_end_flag = 0; //�?帧数据接收完成标�?
uint8_t rx1_buffer[BUFFER_SIZE]={0};  //接收数据缓存数组
//串口2 （BC260�?
volatile uint8_t rx2_len = 0;  //USART2接收�?帧数据的长度
volatile uint8_t rec2_end_flag = 0; //�?帧数据接收完成标�?
...
{   
   ...
   if((tmp_flag != RESET))//idle标志被置�?
   {
   		...
   		rx1_len =  BUFFER_SIZE - temp; //总计数减去未传输的数据个数，得到已经接收的数据个�?
   		rec1_end_flag = 1;	// 接受完成标志位置1	
   }
}

void Usart1_Handle()     //USART1对接收的�?帧数据进行处�?
{
   DMA_Usart1_Send(rx1_buffer, rx1_len);  //将接收到的数据回发给发�?�端
   rx1_len = 0;//清除计数
...
{   
   ...
   if((tmp_flag != RESET))//idle标志被置�?
   {
   		...
   		HAL_UART_DMAStop(&huart2); //  停止DMA传输，防�?
   		temp  =  __HAL_DMA_GET_COUNTER(&hdma_usart2_rx);// 获取DMA中未传输的数据个�?   
   		rx2_len =  BUFFER_SIZE - temp; //总计数减去未传输的数据个数，得到已经接收的数据个�?
   		rec2_end_flag = 1;	// 接受完成标志位置1	
   }
}

void Usart2_Handle()     //USART1对接收的�?帧数据进行处�?
{
   DMA_Usart2_Send(rx2_buffer, rx2_len);  //将接收到的数据回发给发�?�端
   rx2_len = 0;//清除计数
   rec2_end_flag = 0;//清除接收结束标志�?
   HAL_UART_Receive_DMA(&huart2,rx2_buffer,BUFFER_SIZE);//重新打开DMA接收
}

void DMA_Usart2_Send(uint8_t *buf,uint8_t len) //串口发�?�封�?
{   
	if(HAL_UART_Transmit_DMA(&huart2,buf,len)!= HAL_OK) //判断是否发�?�正常，如果出现异常则进入异常中断函�?
	{
  		Error_Handler();
 	}
}
