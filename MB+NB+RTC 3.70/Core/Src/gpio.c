...
{
  //传感�?1�?2，断�?
	W25QXX_Init();//W25Q128初始�?
	 MX_USART3_UART_Init();  
	 MX_UART5_Init();
	 
	int iwdg_flag=1;
//	W25QXX_Write((uint8_t*)&iwdg_flag, IWDG_ADDR, sizeof(int));
	HAL_GPIO_WritePin(GPIOC, SENS1_PWR_EN_Pin, GPIO_PIN_RESET);//传感�?1供电
    HAL_GPIO_WritePin(GPIOA, SEN2_PWR_EN_Pin, GPIO_PIN_RESET);//传感�?2供电
	...
	//读取设备配置信息
	  ...
	  if(iwdg_flag==0)
	  {
		...
		GPIO_AnalogState_Config(); //设置IO口为模拟输入状�??
...
int Tilt_check(void)
{
	...
	short x,y,z;
	int xang,yang,zang;	
	ADXL345_Read_Average(&x,&y,&z,3);  //读取x,y,z 3个方向的加�?�度�? 总共10�?	
